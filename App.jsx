...
// WCS — probeer CORS-vriendelijke geo.api eerst, dan geoservices als fallback
const WCS_URLS  = [
...
// ════════════════════════════════════════════════════════════════════════════
//  INLINE LAMBERT72 CONVERSIE (geen externe bibliotheek nodig)
//  Volledige implementatie: WGS84 → Helmert BD72 → LCC Lambert72
// ════════════════════════════════════════════════════════════════════════════
function wgs84ToLambert72(latDeg, lngDeg) {
  const toRad = d => d * Math.PI / 180;
  const lat = toRad(latDeg), lng = toRad(lngDeg);

  // 1. WGS84 geografisch → ECEF
  ...
  // 2. Helmert 7-parameter: WGS84 → BD72
  const tx=-106.869, ty=52.2978, tz=-103.724;
  const rx=toRad(0.3366/3600), ry=toRad(-0.457/3600), rz=toRad(1.8422/3600);
  const ds=-1.2747e-6, s=1+ds;
  const Xb=s*(X+rz*Y-ry*Z)+tx;
  const Yb=s*(-rz*X+Y+rx*Z)+ty;
  const Zb=s*(ry*X-rx*Y+Z)+tz;

  // 3. ECEF → geografisch op Internationaal (Hayford) ellipsoïd
  const aI=6378388.0, fI=1/297.0, e2I=2*fI-fI*fI, eI=Math.sqrt(e2I);
  ...
  // 4. Lambert72 LCC projectie
  const phi1=toRad(49.8333333), phi2=toRad(51.1666667);
  const lam0=toRad(4.3674867);
...
// ════════════════════════════════════════════════════════════════════════════
//  INLINE GEOTIFF PARSER (Float32 — geen externe bibliotheek nodig)
// ════════════════════════════════════════════════════════════════════════════
function parseTIFF(buf) {
  if (buf.byteLength < 8) throw new Error("Buffer te klein voor TIFF");
  const dv = new DataView(buf);
  const bo = dv.getUint8(0)===0x49; // II=little-endian, MM=big-endian
  const u16=o=>dv.getUint16(o,bo), u32=o=>dv.getUint32(o,bo);
  const f32=o=>dv.getFloat32(o,bo), i16=o=>dv.getInt16(o,bo);
  if((u16(0)!==0x4949&&u16(0)!==0x4D4D)||u16(2)!==42)
    throw new Error("Geen geldige TIFF (magic bytes kloppen niet)");

  let ifo=u32(4); const nTags=u16(ifo); ifo+=2;
  let W=0,H=0,bps=32,sfmt=3;
  let soffs=[],sbytes=[],toffs=[],tbytes=[],tw=0,th=0;

  const getVals=(type,cnt,vp)=>{
    const sz={1:1,2:1,3:2,4:4}[type]||4;
    if(cnt*sz<=4){
      if(type===3) return Array.from({length:cnt},(_,i)=>u16(vp+i*2));
      return Array.from({length:cnt},(_,i)=>u32(vp+i*4));
    }
    const off=u32(vp);
    if(type===3) return Array.from({length:cnt},(_,i)=>u16(off+i*2));
    return Array.from({length:cnt},(_,i)=>u32(off+i*4));
  };

  for(let i=0;i<nTags;i++){
    const t=ifo+i*12, tag=u16(t), type=u16(t+2), cnt=u32(t+4);
    const vs=getVals(type,cnt,t+8), v0=vs[0];
  ...
  if(!W||!H) throw new Error(`TIFF afmetingen ongeldig: ${W}×${H}`);
  const total=W*H, bpSamp=bps/8;
  const data=new Float32Array(total).fill(NaN);
  const rd=o=>sfmt===3&&bps===32?f32(o):sfmt===1&&bps===16?u16(o):sfmt===2&&bps===16?i16(o):f32(o);

  if(toffs.length>0){
    // Tiled TIFF
    const nTilesX=Math.ceil(W/tw);
    toffs.forEach((to,ti)=>{
      const tc=ti%nTilesX, tr=Math.floor(ti/nTilesX);
      for(let r=0;r<th;r++) for(let c=0;c<tw;c++){
        const px=tc*tw+c, py=tr*th+r;
        if(px<W&&py<H) data[py*W+px]=rd(to+(r*tw+c)*bpSamp);
      }
    });
  } else {
    // Stripped TIFF
    let idx=0;
    soffs.forEach((so,si)=>{
      const ns=Math.round(sbytes[si]/bpSamp);
      for(let j=0;j<ns&&idx<total;j++) data[idx++]=rd(so+j*bpSamp);
    });
  }
  return {data,w:W,h:H};
}
...
// ════════════════════════════════════════════════════════════════════════════
//  WCS FETCH — probeert meerdere endpoints, retourneert parsed raster
// ════════════════════════════════════════════════════════════════════════════
async function fetchWCSRaster(xmin,ymin,xmax,ymax,mw,mh,coverage){
  ...
  for(const url of WCS_URLS){
    try{
      const r=await fetch(`${url}?${params}`,{mode:"cors"});
      if(!r.ok){lastErr=`HTTP ${r.status} van ${url}`;continue;}
      const ct=r.headers.get("content-type")||"";
      if(ct.includes("xml")||ct.includes("html")){
        const txt=await r.text();
        lastErr=`WCS servicefout: ${txt.substring(0,120)}`;
        continue;
      }
      const buf=await r.arrayBuffer();
      console.log(`✅ WCS ${coverage} van ${url}: ${buf.byteLength} bytes`);
      return parseTIFF(buf);
    }catch(e){
      lastErr=`${url}: ${e.message}`;
      console.warn(`WCS poging mislukt:`,e.message);
    }
  }
...
// ════════════════════════════════════════════════════════════════════════════
//  HORN'S METHODE — helling & aspect per pixel, dakvlakken clusteren
// ════════════════════════════════════════════════════════════════════════════
function computeRoofFaces(dsmD,dtmD,w,h,cellSize){
  ...
  for(let y=1;y<h-1;y++) for(let x=1;x<w-1;x++){
    ...
    if(slope<6) continue; // vlak dak overslaan
  ...
  pts.forEach(({slope,aspect,relH})=>{
    ...
    bins[b].n++; bins[b].slopes.push(slope); bins[b].heights.push(relH);
  });
...
// ════════════════════════════════════════════════════════════════════════════
//  HOOFD DHM-ANALYSE — bounding box van GRB-contour → WCS → Horn
// ════════════════════════════════════════════════════════════════════════════
async function analyzeDHM(buildingCoords){
  const lats=buildingCoords.map(p=>p[0]), lngs=buildingCoords.map(p=>p[1]);
  const swL=wgs84ToLambert72(Math.min(...lats)-0.0001, Math.min(...lngs)-0.0001);
  const neL=wgs84ToLambert72(Math.max(...lats)+0.0001, Math.max(...lngs)+0.0001);
  const pad=6;
  ...
  console.log(`DHM analyse: bbox L72 ${Math.round(xmin)},${Math.round(ymin)},${Math.round(xmax)},${Math.round(ymax)} → ${mw}×${mh}px, cell=${cellSize.toFixed(1)}m`);

  const[dsmR,dtmR]=await Promise.all([
    fetchWCSRaster(xmin,ymin,xmax,ymax,mw,mh,"DHMVII_DSM_1m"),
    fetchWCSRaster(xmin,ymin,xmax,ymax,mw,mh,"DHMVII_DTM_1m"),
  ]);
  const faces=computeRoofFaces(dsmR.data,dtmR.data,dsmR.w,dsmR.h,cellSize);
  console.log("Gedetecteerde dakvlakken:", faces);
  return faces;
}
...
function clipPolyByLat(poly,keepBelow,midLat){
  ...
  for(let i=0;i<poly.length;i++){
    ...
    const aS=keepBelow?a[0]<=midLat:a[0]>=midLat, bS=keepBelow?b[0]<=midLat:b[0]>=midLat;
    if(aS) out.push(a);
    if(aS!==bS){const t=(midLat-a[0])/(b[0]-a[0]);out.push([midLat,a[1]+t*(b[1]-a[1])]);}
  }
...
// ════════════════════════════════════════════════════════════════════════════
//  GRB GEBOUWCONTOUR — selecteer het gebouw dat de punt STRIKT bevat
//  en het kleinste is (= vermijdt grote samengestelde polygonen)
// ════════════════════════════════════════════════════════════════════════════
async function fetchGRBBuilding(lat,lng){
  ...
  if(!r.ok) throw new Error(`GRB HTTP ${r.status}`);
  return r.json();
}
...
function findBuilding(geojson,lat,lng){
  ...
  for(const f of geojson.features){
    ...
    for(const ring of rings){
      if(pointInPoly([lng,lat],ring)){
        const lc=geoToLeaflet(ring);
        candidates.push({f, area:polyAreaM2(lc), lc});
      }
    }
  }
  // Geef het kleinste gebouw dat de punt bevat (vermijdt samengestelde grote polygonen)
  if(candidates.length>0){
    candidates.sort((a,b)=>a.area-b.area);
    return candidates[0].f;
  }
  // Fallback: dichtstbijzijnd gebouw
...
.sl-item label span.cyan{color:var(--alpha);}
...
.orient-btn{padding:5px 2px;background:var(--bg3);border:1px solid var(--border);border-radius:4px;color:var(--muted);font-family:'IBM Plex Mono',monospace;font-size:8px;cursor:pointer;text-align:center;transition:all .15s;position:relative;}
.orient-btn.active{background:var(--amber-glow);border-color:var(--amber);color:var(--amber);}
.orient-btn.dhm-hit{border-color:rgba(34,211,238,.5);color:rgba(34,211,238,.7);}
.dhm-dot{position:absolute;top:2px;right:2px;width:4px;height:4px;background:var(--alpha);border-radius:50%;}
...
.info-box.err{background:rgba(248,113,113,.07);border-color:rgba(248,113,113,.3);}
...
.face-btn{padding:5px 9px;background:var(--bg3);border:1px solid var(--border);border-radius:5px;color:var(--muted);font-family:'IBM Plex Mono',monospace;font-size:8px;cursor:pointer;transition:all .15s;text-align:left;}
...
.face-btn .fb-main{font-family:'Syne',sans-serif;font-size:11px;font-weight:700;display:block;}
...
.map-legend{position:absolute;bottom:26px;left:10px;z-index:999;background:rgba(10,10,15,.92);border:1px solid var(--border);border-radius:5px;padding:7px 9px;font-family:'IBM Plex Mono',monospace;font-size:8px;backdrop-filter:blur(8px);min-width:160px;}
...
export default function App(){
  ...
  // DHM state — gedetailleerde foutmelding voor debugging
  const[dhmStatus,setDhmStatus]=useState("idle");
  const[dhmError,setDhmError]=useState("");
  ...
  const selectAddr=async(item)=>{
    ...
    try{
      ...
      if(bld){
        ...
        setDetectedArea(area);setBuildingCoords(lCoords);setCustomCount(null);
        setGrbStatus("ok");
        console.log(`GRB: gebouw gevonden, ${area} m²`);
      } else { setGrbStatus("fallback"); }
    }catch(e){console.warn("GRB:",e);setGrbStatus("fallback");}

    // Fallback bounding box als GRB faalt
    const bc=lCoords||(()=>{
      const mLat=111320,mLng=111320*Math.cos(lat*Math.PI/180);
    ...
    // ── Stap 2: DHM LiDAR analyse (volledig inline, geen externe bibliotheek) ──
    ...
    try{
      ...
      if(faces&&faces.length>0){
        ...
        setDhmStatus("error");
        setDhmError("Te weinig dakvlakpixels gevonden (plat dak of te kleine bounding box)");
      }
    }catch(e){
      console.error("DHM analyse fout:",e);
      setDhmStatus("error");
      setDhmError(e.message||"Onbekende fout");
    }
  };
  ...
  const calculate=async()=>{
    ...
    try{
      const dhmStr=dhmStatus==="ok"&&detectedFaces
        ?`\nDHM LiDAR (inline parser, Lambert72): ${detectedFaces.map(f=>`${f.orientation} ${f.slope}° ${f.avgH}m hoogte (${f.pct}%)`).join(", ")}`
        :"\nHandmatige helling/richting.";
  ...
  <div className="app">
    <header className="header">
      ...
      <div className="header-text">
        ...
        <p>GRB Gebouwcontouren · DHM Vlaanderen II LiDAR · Lambert72 · AlphaESS G3</p>
      </div>
    ...
    <div className="main">
      <aside className="sidebar">
        ...
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:5}}>
            ...
            {grbStatus==="ok"&&<div className="info-box grb-ok"><strong>✅ GRB contour geladen</strong> · {detectedArea} m² (gemeten)</div>}
        ...
        {(dhmStatus!=="idle")&&(
          <div>
            <div className="sl">LiDAR Dakanalyse <span style={{color:"var(--alpha)",fontSize:7}}>· Inline · Geen externe libs</span></div>
            {dhmStatus==="loading"&&(
              <div className="info-box" style={{flexDirection:"column",gap:5}}>
                <div style={{display:"flex",alignItems:"center",gap:7}}><div className="spinner cyan"/>WCS ophalen + TIFF parsen + Horn's methode...</div>
                <div className="dhm-bar"><div className="dhm-bar-fill"/></div>
                <div style={{fontSize:7,color:"var(--muted)",marginTop:2}}>Lambert72 conversie · DSM − DTM · aspect/slope clustering</div>
              </div>
            )}
            {dhmStatus==="ok"&&detectedFaces&&(
              <div>
                <div className="info-box dhm-ok" style={{marginBottom:5}}>
                  <strong>✅ {detectedFaces.length} dakvlak{detectedFaces.length>1?"ken":""} gedetecteerd</strong> via LiDAR
                </div>
                <div className="face-grid">
                  {detectedFaces.map((f,i)=>{
                    ...
                    return(<button key={i} className={`face-btn ${selFaceIdx===i?"active":""}`} onClick={()=>selectFace(i,detectedFaces)}>
                      <span className="fb-main">{f.orientation} · {f.slope}°</span>
                      <span className="fb-sub">{f.pct}% v. dak · gem. {f.avgH}m</span>
            ...
            {dhmStatus==="error"&&(
              <div className="info-box err">
                ...
                <span style={{fontSize:7,wordBreak:"break-all"}}>{dhmError||"Onbekende fout"}</span><br/>
        ...
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {grbStatus==="ok"
              ?<div style={{padding:"5px 9px",background:"var(--bg3)",border:"1px solid var(--border)",borderRadius:5,fontSize:8,color:"var(--muted)"}}>Oppervlak: <span style={{color:"var(--green)",fontWeight:600}}>{detectedArea} m²</span> (GRB gemeten)</div>
            ...
            <div className="sl-item">
              <label>Hellingshoek <span className={dhmStatus==="ok"?"cyan":""}>{slope}° {dhmStatus==="ok"?"· LiDAR":""}</span></label>
        ...
        <div>
          ...
          {coords&&<div style={{display:"flex",gap:5,marginTop:5}}>
            <div style={{flex:1,padding:"4px 7px",background:`${zq[0].c}22`,border:`1px solid ${zq[0].c}55`,borderRadius:4,fontSize:7,color:zq[0].c}}>Z: {zq[0].l}</div>
            <div style={{flex:1,padding:"4px 7px",background:`${zq[1].c}22`,border:`1px solid ${zq[1].c}55`,borderRadius:4,fontSize:7,color:zq[1].c}}>N: {zq[1].l}</div>
          </div>}
        </div>
        <div className="divider"/>
        ...
        <button className="btn full" onClick={calculate} disabled={!coords||aiLoading||!buildingCoords||isLoading}>
          {aiLoading?<><div className="spinner"/>Analyseren...</>:
           dhmStatus==="loading"?<><div className="spinner cyan"/>LiDAR verwerken...</>:
           grbStatus==="loading"?<><div className="spinner"/>Gebouw laden...</>:
           "☀️ Bereken & toon panelen op dak"}
        </button>
        <div className="info-box">
          <strong>📡 Databronnen</strong><br/>
          GRB · Reële gebouwcontouren<br/>
          DHM II WCS · DSM+DTM · 1m res. · Horn<br/>
          Lambert72 · Helmert 7-param · Inline<br/>
      ...
      <div className="content-area">
        {activeTab==="configuratie"&&(
          <div className="map-area">
            ...
            {coords&&<div className="status-pill">
              ...
              {grbStatus==="fallback"&&<span style={{color:"var(--amber)"}}>GRB ⚠️</span>}
            ...
            {coords&&<div className="map-legend">
              ...
              {dhmStatus==="ok"&&<div style={{marginTop:3,color:"var(--alpha)",fontSize:7}}>◉ = LiDAR-gedetecteerde richting</div>}
            </div>}
          </div>
        )}
        ...
        {activeTab==="omvormers"&&<div className="section">
          ...
          {selInv&&<div style={{display:"flex",justifyContent:"flex-end"}}><button className="btn sec sm" onClick={()=>setSelInvId(null)}>✕ Verwijder</button></div>}
        ...
        {activeTab==="resultaten"&&(results?(
          <div className="results-wrap">
            <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
              {results.grbOk&&<div style={{padding:"4px 9px",background:"rgba(34,197,94,.07)",border:"1px solid rgba(34,197,94,.3)",borderRadius:4,fontSize:7,color:"var(--green)"}}>✅ GRB dakcontour · {results.detectedArea} m²</div>}
              {results.dhmOk&&<div style={{padding:"4px 9px",background:"var(--alpha-glow)",border:"1px solid rgba(34,211,238,.3)",borderRadius:4,fontSize:7,color:"var(--alpha)"}}>✅ LiDAR helling · {results.orientation} {results.slope}° · Inline parser</div>}
            </div>
            ...
            <div style={{padding:"5px 10px",background:"rgba(29,78,216,.1)",border:"1px solid rgba(96,165,250,.3)",borderRadius:5,fontSize:8,color:"#60a5fa"}}>
              🗺️ <strong>Configuratie tab</strong> — {results.panelCount} panelen op het reële dak. +/− past het aantal live aan.
            </div>
          ...
          <div className="empty-state">
            ...
            <p>Voer een adres in. De app laadt de GRB-dakcontour en analyseert de dakhellingen via DHM Vlaanderen LiDAR — volledig inline, zonder externe bibliotheken.</p>
          </div>
        ))}
      </div>
    </div>
